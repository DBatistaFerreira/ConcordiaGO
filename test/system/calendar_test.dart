...
void main() {
  ...
  MaterialApp calendarsPageWidget = MaterialApp(
      home: BlocProvider<CalendarBloc>(
          ...
          child: CalendarsPage(deviceCalendarPlugin, key: Key('calendarsPage'))
      )
  );
  final calendars = Result<UnmodifiableListView<Calendar>>();
  calendars.data = UnmodifiableListView([Calendar(name: "test", isReadOnly: false, isDefault: true, color: 1, id:"123", accountType: "google", accountName: "test")]);
  ...
  testWidgets('Choose Event Page Widget', (WidgetTester tester) async {
    ...
    result.data = UnmodifiableListView([Event("test",eventId: "123", end: DateTime.now(), start: DateTime.now(), allDay: false, attendees: [], description: "test", title: "class 101", recurrenceRule: null)]);

    var widget = MaterialApp(
        home: BlocProvider<CalendarBloc>(
          create: (context) => CalendarBloc(),
          child: CalendarEventsPage(Calendar(accountName: "test", accountType: "test", color: 1, id: "1234", isDefault: true, isReadOnly: false, name: "Test"),deviceCalendarPlugin, key: Key('calendarsPage')),
    ));
