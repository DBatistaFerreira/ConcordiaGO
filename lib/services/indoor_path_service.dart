...
class IndoorPathService {
  static String parse(String documentFile, List<List<int>> nodeCoordinates) {
    final xml.XmlDocument document = xml.parse(documentFile);
    final List<xml.XmlNode> textGroup = document.findAllElements('text').first.parent.children;
    ...
    for (int i = 0; i < nodeCoordinates.length - 1; i++) {
      addLine(textGroup, nodeCoordinates[i][0], nodeCoordinates[i][1], nodeCoordinates[i + 1][0],
          nodeCoordinates[i + 1][1]);
    }
  ...
  static void addLine(List<xml.XmlNode> targetGroup, int x1, int y1, int x2, int y2) {
    targetGroup.add(xml.XmlElement(xml.XmlName.fromString('line'), <xml.XmlAttribute>[
  ...
  static void addStartTag(List<xml.XmlNode> targetGroup, int x, int y) {
    targetGroup.add(xml.XmlElement(xml.XmlName.fromString('path'), <xml.XmlAttribute>[
